...
<td style="padding:3px 0"><div style="background:${e.estrellas <= d.umbral_negativa ? ALERTA : VERDE};height:10px;border-radius:5px;width:${Math.round((e.total / maxBarra) * 100)}%;min-width:${e.total ? '6px' : '0'}"></div></td>
...
const flecha = r.tendencia ? (r.tendencia.media > 0 ? `<span style="color:${OK}">▲ +${r.tendencia.media}</span>`
  : (r.tendencia.media < 0 ? `<span style="color:${ALERTA}">▼ ${r.tendencia.media}</span>` : '=')) : '';
...
${cifra(r.media === null ? '—' : r.media, `nota media ${flecha}`)}${cifra(r.total_semana, 'reseñas')}${cifra(r.negativas, 'quejas', r.negativas ? ALERTA : null)}${cifra(r.positivas, 'elogios', r.positivas ? OK : null)}
</tr></table>
${h3('Reparto de estrellas')}
<table width="100%" cellspacing="0">${barras}</table>
${h3('De qué se queja la gente')}
${temas}
${r.peores && d.peores.length ? `${h3('Para contestar')}${d.peores.map((x) => cita(x, ALERTA)).join('')}` : ''}
...
const telegram = `⭐ <b>Reseñas de la semana · ${d.tienda}</b>\n` +
  `<b>Nota media:</b> ${r.media === null ? '—' : r.media}${r.tendencia ? ` (${r.tendencia.media > 0 ? '+' : ''}${r.tendencia.media} vs semana anterior)` : ''}\n` +
...
return [{ json: { ...d, asunto: `Denoro · Reseñas de ${d.tienda}: media ${r.media === null ? '—' : r.media} (${r.total_semana} esta semana)`, email_html, telegram } }];

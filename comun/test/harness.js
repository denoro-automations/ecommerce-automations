...
// carpeta: ruta de la automatización (la que contiene src/)
function crearRunCode(carpeta) {
  return function runCode(file, { input = [], nodes = {}, staticData = {}, replace = [] } = {}) {
    let code = fs.readFileSync(path.join(carpeta, 'src', file), 'utf8');
    ...
    const fn = new Function('$', '$input', '$getWorkflowStaticData', 'Buffer', 'URL',
      `return (async () => {${code}\n})();`);
    return fn($, wrap(input), () => staticData, Buffer, undefined); // n8n no expone URL
  };
}
module.exports = { crearRunCode };
